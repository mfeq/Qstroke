...
fn ffbasepoint_to_point(me: fontforge::BasePoint, prevcp: Option<fontforge::BasePoint>, nextcp: Option<fontforge::BasePoint>, should_move: bool) -> glifparser::Point<()> {
    let mut p = glifparser::Point::new();
    p.x = me.x as f32;
    p.y = me.y as f32;
    p.b = ffbasepoint_to_handle(prevcp);
    p.a = ffbasepoint_to_handle(nextcp);
    p.ptype = if should_move { glifparser::PointType::Move } else { glifparser::PointType::Curve };
    p
}
...
pub fn convert_glif(settings: &NibSettings) -> Option<String> {
    ...
    let nibglif: glifparser::Glif<()> = glifparser::read_ufo_glif(&fs::read_to_string(&settings.nib).expect("Nib .glif inaccessible"));
    if !settings.quiet {
        eprintln!("Reading path...");
    }
    let ssglif: glifparser::Glif<()> = glifparser::read_ufo_glif(&fs::read_to_string(&settings.path).expect("Path to stroke .glif inaccessible"));
